...
/* ================================        MAIN       ===================================*/
	...
	TFile root_file("pi.root", "RECREATE");
...
if (mc)
	...
	GammaFinder gamma_finder(tree, true);
	PiGGFinder pi_finder(tree,gamma_finder);
	...
	while (nextEvent()) {
		...
		if (!pi_finder.find())
			continue;
		while (pi_finder.next()){
			std::cout << pi_finder.getMass() << ' ' << std::endl;
			pi_finder.fill();
			evt_saver.fill();
			tree.Fill();
		}
		AA::dst.outEventLst("pi_elist");
...
if (mc)
	...
	root_file.Write();
	root_file.Close();
	std::cout << argv[0] << " II: pi_finder ended succesfully." << std::endl;
