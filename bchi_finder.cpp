...
/* ================================        MAIN       ===================================*/
	...
	TFile root_file("bchi.root", "RECREATE");
...
if (mc)
	...
	ChiJPsiGFinder chi_finder(tree, jpsi_finder, gamma_finder);
	PtlFinder kaon_finder("k",tree,true);
	BChiKFinder b_finder(tree,chi_finder, kaon_finder);
	...
	while (nextEvent()) {
		...
		if (!b_finder.find())
			continue;
		while (b_finder.next()){
			std::cout << "B: " << b_finder.getMass() << ' ' << std::endl;
			b_finder.fill();
			evt_saver.fill();
			tree.Fill();
		}
		AA::dst.outEventLst("bchi_elist");
